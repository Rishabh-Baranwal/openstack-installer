...
#my_ip_pref=`ifconfig $interface | grep "inet " | awk '{split($2, a, "."); printf("%s.%s.%s.", a[1], a[2], a[3]);}'`

my_ip_pref=`echo ${1} | awk '{split($0, a, "."); printf("%s.%s.%s.", a[1], a[2], a[3]);}'`

# for floating IP and external connectivity
# choose a small pool from the subnet from ens4
POOL_START=${my_ip_pref}${2:-100}
POOL_END=${my_ip_pref}${3:-120}
GW=${my_ip_pref}1
